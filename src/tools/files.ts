...
import { deliverBinary, mediaInputSchema, resolveMedia, toBuffer } from "../media.js";
...
export const filesTools: ToolDef[] = [
  ...
  tool({
    ...
    description:
      "Download the binary content of a non-Google-Docs file (alt=media). For Google Docs/Sheets/Slides use drive_files_export instead. Text content is returned as text; other types as base64, or written to disk when destinationPath is given.",
    readOnly: true,
    idempotent: true,
    inputSchema: {
      fileId: z.string().describe("The ID of the file."),
    ...
    async handler(args, ctx) {
      ...
      const meta = await drive.files.get(
        withDriveDefaults({ fileId: args.fileId, fields: "name,mimeType,size", supportsAllDrives: args.supportsAllDrives }),
      );
  ...
  tool({
    ...
    inputSchema: {
      ...
      destinationPath: z
        .string()
        .optional()
        .describe("Absolute path to write the exported bytes to. Requires DRIVE_ALLOW_LOCAL_FILES=1."),
    },
    async handler(args, ctx) {
      const drive = await ctx.drive();
      ...
      const bytes = toBuffer(res.data);
      return deliverBinary(bytes, args.mimeType, `googledrive:///${args.fileId}`, ctx.config, args.destinationPath);
    },
  }),
  ...
  tool({
    ...
    description:
      "Start a long-running download of a file or its exported form, returning an Operation. Poll it with drive_operations_get; the finished operation carries a partialDownloadAllowed flag and a downloadUri. This is the route for exports over 10 MB.",
    readOnly: true,
    openWorld: true,
    inputSchema: {
  ...
  tool({
    ...
    description:
      "Create a new file, folder or shortcut. Set metadata.mimeType to 'application/vnd.google-apps.folder' for a folder. Supply `media` to upload content; set metadata.mimeType to a Google Docs type to convert the upload (e.g. upload text/csv as application/vnd.google-apps.spreadsheet).",
    inputSchema: {
  ...
  tool({
    ...
    inputSchema: {
      ...
      media: mediaInputSchema.optional().describe("New content for the file. Omit to change metadata only."),
